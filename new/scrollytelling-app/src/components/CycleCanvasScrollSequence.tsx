...
export default function CycleCanvasScrollSequence() {
    ...
    const loadImagesParallel = async () => {
      const promises = Array.from({ length: TOTAL_FRAMES }, (_, i) => {
        return new Promise<{img: HTMLImageElement, index: number}>((resolve) => {
          ...
          // Update frame path specifically to cycle-sequence
          img.src = `/images/cycle-sequence/ezgif-frame-${frameIndex}.jpg`;
    ...
    <div className="w-full h-full relative text-white/90 font-sans mx-auto max-w-7xl px-8 sm:px-12">
      ...
      >
        ...
        <p className="text-lg sm:text-xl text-white/50 font-light max-w-xl mx-auto">
          Unleash the ultimate outdoor performance. Scroll slowly to dissect the mechanics.
        </p>
      </motion.div>
      ...
      >
        ...
        <h2 className="text-3xl sm:text-5xl md:text-6xl font-bold mb-4 tracking-tight">Carbon Fiber Frame</h2>
        <p className="text-base sm:text-lg text-white/60 max-w-xs leading-relaxed">
          Aerospace-grade materials provide absolute rigidity while shedding unnecessary weight dynamics.
        </p>
      </motion.div>
      ...
      >
        <div className="w-12 h-[2px] bg-[#00FF9C] mb-6"></div>
        <h2 className="text-3xl sm:text-5xl md:text-6xl font-bold mb-4 tracking-tight">Adaptive Suspension</h2>
        <p className="text-base sm:text-lg text-white/60 max-w-xs leading-relaxed">
          Micro-adjustable shocks intuitively read the terrain, providing flawless kinetic energy dispersion.
        </p>
      </motion.div>
