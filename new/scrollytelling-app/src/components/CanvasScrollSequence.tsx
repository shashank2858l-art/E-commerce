    ...
    const loadImagesParallel = async () => {
      const promises = Array.from({ length: TOTAL_FRAMES }, (_, i) => {
        return new Promise<{img: HTMLImageElement, index: number}>((resolve) => {
          ...
          img.src = `/images/sequence/ezgif-frame-${frameIndex}.jpg`;
      ...
      {!isLoaded && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black">
          <div className="flex flex-col items-center">
             ...
             <div className="text-white text-sm tracking-[0.3em] font-light uppercase">Constructing Experience</div>
          </div>
        </div>
      )}
    ...
    <div className="w-full h-full relative text-white/90 font-sans mx-auto max-w-7xl px-8 sm:px-12">
      ...
      >
        ...
        <p className="text-lg sm:text-xl text-white/50 font-light max-w-xl mx-auto">
          Next-generation web3 architecture. Scroll slowly to dissect the mechanics.
        </p>
      </motion.div>
      ...
      >
        ...
        <h2 className="text-3xl sm:text-5xl md:text-6xl font-bold mb-4 tracking-tight">Titanium Core</h2>
        <p className="text-base sm:text-lg text-white/60 max-w-xs leading-relaxed">
          Unprecedented power distribution layered seamlessly within a fully recycled Grade 5 thermal chassis.
        </p>
      </motion.div>
      ...
      >
        <div className="w-12 h-[2px] bg-[#00FF9C] mb-6"></div>
        <h2 className="text-3xl sm:text-5xl md:text-6xl font-bold mb-4 tracking-tight">Zero-G Engine</h2>
        <p className="text-base sm:text-lg text-white/60 max-w-xs leading-relaxed">
          Every microscopic component engineered for maximal kinetic efficiency and autonomous heat dispersion.
        </p>
      </motion.div>
