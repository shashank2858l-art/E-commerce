                ...
                {procuredItems.filter(i => i.type === viewMode || (viewMode === 'buy' && !['rent', 'swap'].includes(i.type))).map((item, idx) => (
                  <div key={idx} className="glass rounded-2xl p-4 flex gap-4 border border-neon-green/20 relative items-center shadow-[0_0_20px_rgba(57,255,20,0.05)]">
                    ...
                    <div className="flex-1 pr-24">
                      ...
                      <div className="text-[10px] text-muted-dim font-mono mb-1">🕒 Acquired {new Date(item.purchasedAt).toLocaleDateString()}</div>
                      <div className="text-[10px] text-neon-green font-mono">📦 Est. 3–7 days delivery</div>
                      
                      {/* Self-Listing Indicator */}
                      {(item.ownerId === currentUser.id) && (
                        <div className="inline-flex items-center gap-1.5 px-2 py-1 rounded bg-neon-green/10 border border-neon-green/20 text-[9px] font-heading font-bold uppercase text-neon-green tracking-wider mt-1">
                           <span>👤</span> Your Own Listing
                        </div>
                      )}
                    </div>
                    
                    <div className="absolute top-1/2 -translate-y-1/2 right-4 flex flex-col gap-2 items-center">
                      {/* Track Button */}
                      <Link href="/tracking" className="bg-accent-gold/20 text-accent-gold p-2 rounded-full hover:scale-110 transition-transform shadow-[0_0_10px_rgba(255,200,0,0.2)]" title="Track Order">
                        🗺️
                      </Link>
                      ...
                      {item.ownerId !== currentUser.id ? (
                        item.sellerWhatsapp ? (
                          <a href={`https://wa.me/${item.sellerWhatsapp}?text=Hi! I procured your ${item.title} on Reuse_mart!`} target="_blank" rel="noopener noreferrer" className="bg-[#25D366] text-white p-2 rounded-full hover:scale-110 transition-transform shadow-[0_0_10px_rgba(37,211,102,0.3)]" title={`Contact ${item.sellerName || 'Seller'} on WhatsApp`}>
                          ...
                          <div className="bg-white/5 text-muted-dim p-2 rounded-full text-[10px]" title="Seller contact not available">📞</div>
                        )
                      ) : (
                        <div className="bg-white/5 text-muted p-2 rounded-full" title="You are the seller">👤</div>
                      )}
                    </div>
                  </div>
                ))}
