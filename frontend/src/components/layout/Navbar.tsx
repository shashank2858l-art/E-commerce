...
export default function Navbar() {
  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-background/80 backdrop-blur-xl border-b border-white/5 shadow-[0_4px_30px_rgba(0,0,0,0.5)]' : 'bg-gradient-to-b from-background/90 to-transparent'
        }`}
    >
      <div className="section-container flex items-center h-16">
        ...
        <div className="hidden md:flex items-center gap-1 lg:gap-3 xl:gap-4 flex-1 justify-center">
          {navKeys.filter(link => !link.authRequired || isAuthenticated).map((link) => {
            ...
            return (
              <LinkTag
                ...
                className={`relative px-3 lg:px-4 py-2 text-[11px] lg:text-xs xl:text-sm font-heading font-semibold tracking-widest uppercase transition-colors rounded-lg z-10 whitespace-nowrap flex items-center gap-1.5 ${isExternal ? 'text-amber-400 hover:text-amber-300' : pathname === link.href ? 'text-neon-green drop-shadow-[0_0_5px_rgba(57,255,20,0.5)]' : 'text-muted-dim hover:text-white'
                  }`}
              >
        ...
        {/* CTA + Actions + Mobile Toggle — right aligned */}
        <div className="flex items-center gap-2 lg:gap-3 flex-shrink-0 ml-6">
          <button
            onClick={toggleTheme}
                ...
                return (
                  ...
                  >
                    <LinkTag
                      ...
                      className={`flex items-center gap-2 px-4 py-3 rounded-xl font-heading font-semibold tracking-widest uppercase text-sm ${isExternal ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' : pathname === link.href ? 'bg-neon-green/10 text-neon-green border border-neon-green/20' : 'text-muted hover:bg-white/5 hover:text-white'
                        }`}
                    >
