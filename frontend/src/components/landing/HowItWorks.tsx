...
const TiltCard = ({ step, index }: { step: any, index: number }) => {
  ...
  return (
    <motion.div
      ...
      className="relative h-[300px] sm:h-[350px] lg:h-[400px] w-full rounded-2xl overflow-hidden group cursor-pointer border border-white/10 hover:border-neon-green/80 transition-colors duration-500 hover:shadow-[0_0_30px_rgba(57,255,20,0.3)] bg-surface-base"
    >
      ...
      <div className="absolute inset-0 z-10 p-5 sm:p-6 lg:p-8 flex flex-col justify-end transform-gpu pointer-events-none" style={{ transform: "translateZ(50px)" }}>
        <span className="text-3xl sm:text-4xl lg:text-5xl mb-3 sm:mb-4 lg:mb-6 block transform-gpu drop-shadow-[0_0_15px_rgba(57,255,20,0.5)]" style={{ transform: "translateZ(80px)" }}>
          {step.icon}
        </span>
...
export default function HowItWorks() {
  ...
  return (
    <section className="py-12 sm:py-16 lg:py-24 relative overflow-hidden bg-background">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 perspective-[1000px]">
        <motion.div
        ...
        >
          ...
          <h2 className="font-heading text-2xl sm:text-3xl lg:text-5xl font-extrabold mt-4 text-white">
            How Reuse_Mart Works
          </h2>
          <p className="mt-3 sm:mt-6 text-muted font-sans text-sm sm:text-base lg:text-xl max-w-2xl mx-auto tracking-wide">
            A smarter way to shop, trade, and reduce waste.
          </p>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 perspective-[1000px] w-full items-center justify-center">
          {steps.map((step, i) => (
